...
- **Exact proportional-distribution rounding** (splitting a category's target value across multiple items, and the resulting quantities against each item's own decimal precision) — a planning-time implementation detail, not a design-level ambiguity.
  **Resolved by implementation (2026-09-06):** no rounding at all. `stock_movements.quantity` and `inventory_stock_balances.quantity_on_hand` are both `NUMERIC` (unbounded decimal precision), and `computeInvoiceDeductionPlan()` never calls `Math.round`/`toFixed`/`Math.floor` anywhere — a deducted quantity is exactly `targetValue / unitCost`, e.g. 163.333... units. This is deliberate: the deduction is value-driven (a % of the invoice's pre-VAT amount), so rounding the resulting quantity would make the posted value drift from the intended cut, violating the feature's own "never fabricate or lose value" invariant. Trade-off accepted: a physically discrete unit (e.g. a "ชิ้น" item like a door handle) can end up with a fractional quantity_on_hand — not a bug, just unresolved for item types where whole-unit counting matters. A future iteration could round only for such categories if this becomes a real complaint, but nothing does that today.
